  ...
  return (
    <div className="flex flex-col h-full w-full bg-background relative overflow-hidden">
      ...
      <main className="flex-1 relative flex flex-col min-h-0 touch-none">
        ...
        {praise && !showLeaderboard && !showRestartOverlay && <div className="absolute inset-0 z-20 flex flex-col items-center justify-center pb-52 bg-black/60 backdrop-blur-sm animate-fade-in pointer-events-none"><div className="animate-pop flex flex-col items-center"><div className="text-6xl mb-4 drop-shadow-[0_0_25px_rgba(255,255,255,0.4)] filter contrast-125 saturate-150 transform hover:scale-110 transition-transform">{praise.emoji}</div><h1 className="text-5xl sm:text-7xl font-black text-transparent bg-clip-text bg-gradient-to-b from-white to-zinc-400 drop-shadow-[0_4px_4px_rgba(0,0,0,0.8)] text-center tracking-tighter leading-none">{praise.text}</h1></div>{winner && <div className="mt-8 flex flex-col items-center animate-slide-up bg-zinc-900/80 p-6 rounded-2xl border border-emerald-500/30 shadow-2xl backdrop-blur-md"><div className="relative"><img src={winner.profilePictureUrl} alt="" className="w-24 h-24 rounded-full border-4 border-emerald-400 shadow-[0_0_30px_rgba(16,185,129,0.4)]" /><div className="absolute -top-3 -right-3 bg-emerald-500 text-white p-2 rounded-full shadow-lg"><Trophy size={20} fill="white" /></div></div><div className="mt-4 text-center"><p className="text-zinc-400 text-sm font-bold uppercase tracking-wider mb-1">WINNER</p><p className="text-2xl font-black text-white drop-shadow-md">{winner.nickname}</p><p className="text-emerald-400 font-bold">@{winner.uniqueId}</p></div></div>}{gameStatus === GameStatus.LOST && <div className="mt-8 flex flex-col items-center animate-slide-up bg-rose-950/80 p-6 rounded-2xl border border-rose-500/30 shadow-2xl backdrop-blur-md"><p className="text-rose-300 text-sm font-bold uppercase tracking-wider mb-2">THE WORD WAS</p><p className="text-4xl font-black text-white tracking-widest drop-shadow-[0_2px_10px_rgba(225,29,72,0.6)]">{targetWord}</p></div>}</div>}
        {showLeaderboard && <div className="absolute inset-0 z-30 bg-black/85 backdrop-blur-xl flex flex-col items-center justify-center p-4 animate-fade-in"><div className="w-full max-w-sm"><div className="flex items-center justify-center gap-3 mb-6"><Crown size={32} className="text-amber-400 drop-shadow-[0_0_15px_rgba(251,191,36,0.5)] animate-bounce" fill="currentColor" /><h2 className="text-3xl font-black text-white tracking-tighter drop-shadow-lg">LEADERBOARD</h2></div><div className="bg-gradient-to-r from-rose-900/50 to-rose-800/50 rounded-xl p-3 mb-4 border border-rose-500/30 flex items-center justify-between shadow-lg"><div className="flex items-center gap-3"><div className="bg-rose-500/20 p-2 rounded-lg"><Flame size={20} className="text-rose-400" /></div><div><p className="text-rose-200 text-xs font-bold uppercase tracking-wider">HOST WINS</p></div></div><span className="text-2xl font-black text-rose-400 drop-shadow-md">{hostScore}</span></div><div className="h-[265px] overflow-hidden"><div className={`transition-transform duration-[2000ms] ease-in-out ${isLeaderboardScrolled ? '-translate-y-[220px]' : 'translate-y-0'}`}><div className="space-y-2">{leaderboard.length > 0 ? leaderboard.map((user, index) => { let rankStyle = "bg-zinc-800/50 border-zinc-700/50 text-zinc-400"; let icon = <span className="font-mono font-bold text-zinc-500 w-5 text-center text-xs">#{index+1}</span>; if (index === 0) { rankStyle = "bg-gradient-to-r from-amber-900/40 to-amber-800/40 border-amber-500/50 shadow-[0_0_15px_rgba(245,158,11,0.1)]"; icon = <Crown size={16} className="text-amber-400" fill="currentColor"/>; } else if (index === 1) { rankStyle = "bg-gradient-to-r from-zinc-800 to-zinc-700 border-zinc-400/30"; icon = <Medal size={16} className="text-zinc-300" />; } else if (index === 2) { rankStyle = "bg-gradient-to-r from-orange-900/30 to-orange-800/30 border-orange-700/30"; icon = <Star size={16} className="text-orange-400" />; } return (<div key={user.userId} className={`flex items-center justify-between p-2 rounded-xl border ${rankStyle} backdrop-blur-sm`}><div className="flex items-center gap-2"><div className="flex items-center justify-center w-6">{icon}</div><img src={user.profilePictureUrl} alt="" className="w-8 h-8 rounded-full border border-white/10" /><div><p className="font-bold text-white text-xs leading-tight">{user.nickname}</p><div className="flex items-center gap-2 mt-0.5"><span className="text-[9px] bg-emerald-500/20 text-emerald-300 px-1 py-0.5 rounded flex gap-1 items-center font-bold"><Trophy size={8}/> {user.wins}</span></div></div></div><div className="text-right"><span className="block text-lg font-black text-white">{user.score}</span><span className="text-[9px] text-zinc-500 font-bold uppercase">PTS</span></div></div>); }) : <div className="text-center pt-12 text-zinc-500 italic border-2 border-dashed border-zinc-800 rounded-xl h-full">No winners yet. Be the first!</div>}</div></div></div></div></div>}
